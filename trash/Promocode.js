  ...
  create = async (req, res) => {
    try {
      ...
      if (permissions && permissions.canCreatePromoCode) {
        ...
        if (p_code)
          return res.status(409).send({ message: "This code Already Exist" });
        let {
          code,
          discount,
          expireAt,
        ...
        res.send({ message: "Promocode created successfully" });
      } else {
      ...
      res.status(500).send({ message: err.message || "Something Failed." });
    }
  };
  ...
  getPromocodes = async (req, res) => {
    try {
      ...
      if (permissions && permissions.canReadPromoCode) {
        ...
        res.send({ message: "Get Promocodes", data: p_code });
      } else {
        return res.status(403).send({
          message: "You don't have permisssions to create Promocode!",
        });
    ...
    try {
      ...
      if (p_code && p_code.quantity) {
        ...
        if (used_code.length == p_code.limit)
          return res.send({ message: "You have already used!" });
        ...
        res.send({ message: "Promocode Applied", data: p_code });
      } else {
        res.status(404).send({ message: "This code does not exist!" });
      }
    } catch (err) {
      res.status(500).send({ message: err.message || "Something Failed." });
    }
