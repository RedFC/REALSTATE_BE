...
class UpdateSuperAdmin {
  ...
  updateMember_ = async ({
    ...
    try {
      ...
      let userDetails = await UsersDetail.findOne({
        raw: true,
        where: {
          userId,
        },
            ...
            if (uploadRslt) {
              ...
              if (foundImgData) {
                ...
                cloudinary.remove(foundImgData.imageId)
                  .then(rmvFile => {
                    if (!rmvFile)
                      throw new Error("An error occured while updating the Image.");
                  })
                  .catch(error => {
              ...
              if (isMerchant) {
                ...
                if (updateMerchnatdetail[0]) {
                  ...
                  if (getallmerchantCategory.length) {
                    res.send(getallmerchantCategory)
                    ...
                    let PresentCategoryId = getCatId.filter(
                      (obj) => compareId.some((obj2, index) => obj == obj2)
                    );
                    let CreateCategoryId = compareId.filter(
                      (obj) => !PresentCategoryId.some((obj2, index) => obj == obj2)
                    );
                    CreateCategoryId.map(x => bulkCreateMerchantCategory.push({
                    ...
                    if (DeletedCategoryId.length) {
                      DeletedCategoryId.map(async x => {
                        ...
                        if (getProducts) {
                          let getCatName = getallmerchantCategory.filter(function (cats) {
